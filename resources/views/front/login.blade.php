            ...
            <div class="row">
                ...
                <div class="col-md-6 mx-auto rgister-hidden active" id="one">
                    <div class="regsiter-client">
                        ...
                        <p>{{__('site.If you have an account, sign in with your email address').' '.__('site.Or Phone')}}</p>
                        @if (session('status'))
                        ...
                        @isset($url)
                            ...
                            @endif
                            <input type="text" name="phone" placeholder="@lang('site.Phone Or Email')" class="mb-4 @if ($errors->has('phone')) is-invalid @endif" value="{{old('phone')}}">
                            ...
                            <button type="submit" class="btn btn-primary mt-4" style="width: fit-content; background-color: #000000; color:#fff;">@lang('site.log in')</button>
                        </form>
                    </div>
                </div>
                            ...
                            <form class="mt-4" method="POST" action='{{ url(app()->getLocale()."/register/$url") }}' aria-label="{{ __('Register') }}">
                                @else
                                    <form method="POST" action="{{ route('register') }}" aria-label="{{ __('Register') }}">
                                        ...
                                        @endif
                                        <input placeholder="{{__('site.Phone')}} " name="phone" class="mb-4 @if ($errors->has('phone')) is-invalid @endif" type="text" minlength="8">
                                        @if ($errors->has('phone'))
                                            <span class="text-danger">{{ $errors->first('phone') }}</span>
                                        @endif
                                        <select style="padding: 5px" name="country" id="country" class=" @if ($errors->has('country')) is-invalid @endif" >
                                        ...
                                        @endif
                            <button type="submit" class="btn btn-primary mt-4" style="width: fit-content; background-color: #000000; color: #fff;">{{__('site.register')}} </button>
                        </form>
                    </div>
                </div>
            </div>
