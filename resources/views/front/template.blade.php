...
@section('content')
   ...
   <section class="py-5">
    <div class="container">
        ...
        <input type="hidden" id="currentType" value="{{ request('type') }}">
...
<script>
    ...
    function loadMoreData() {
        ...
        $.ajax({
            url: '?page=' + page + '&type=' + encodeURIComponent(type) + '&sort=' + encodeURIComponent(sort),
            type: "GET",
            dataType: "json",
            beforeSend: function () {
                $('#loader').show();
 ...
 <script>
  $(document).ready(function () {
    $('#selectSort').on('change', function (e) {
      ...
      // تأكد من الحفاظ على النوع type إن وُجد
      const type = $('#currentType').val();
      if (type) {
