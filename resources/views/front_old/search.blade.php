            ...
            <div class="container">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-site py-0 d-flex justify-content-center">
                        ...
                        <li class="breadcrumb-item active pl-0 d-flex align-items-center" aria-current="page">@lang('site.Search results')
                        </li>
                    </ol>
                </nav>
            </div>
            ...
            <div class="container container-xl">
                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap">
                    <p class="fs-18 font-weight-500 my-lg-0 my-2" style="color: #696969;"><strong
                                class="font-weight-bold text-secondary">{{ $items->total() }}</strong>
                        @lang('site.product')
                    </p>
                ...
                <div class="row mb-3">
                    @if ($items->count() >= 1)
                        @foreach ($items as $one)
                            <div class="col-lg-3 col-xl-3 col-md-4 col-6 mb-3 "   >
          ...
          >
                                    ...
                                    <div class="card-body pt-4 text-center m-auto m-md-1 d-block  justify-content-between align-items-center">
                                        ...
                                        @if($one->if_sale)
                                            ...
                                            $discount=100 - round(($sale/$regular)*100,1);
                                            ?>
                                            <p class="card-text font-weight-bold fs-16 mb-1 text-decoration-through">{{get_price_helper($one->regular_price,true)}}</p>
                                            <p class="card-text font-weight-bold fs-16 mb-1 text-secondary">{{ get_price_helper($one->sale_price,true) }}</p>
                                        @else
                                            <p class="card-text font-weight-bold fs-16 mb-1 text-white">{{get_price_helper($one->regular_price,true)}}</p>

                                            <p class="card-text font-weight-bold fs-16 mb-1 text-secondary">{{get_price_helper($one->regular_price,true)}}</p>
                                        @endif
                                        <p class="d-flex">
                                            ...
                                            <a href="{{route("product",$one->id)}}" class="fs-16 w-75 ml-auto d-flex align-items-center justify-content-center text-secondary bg-white hover-white bg-hover-secondary  mb-2  rounded-top rounded-bottom">اشتري الان</a>
                                        </p>
                                    </div>
                                </div>
