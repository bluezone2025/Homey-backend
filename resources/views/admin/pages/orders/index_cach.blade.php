...
@section('content')
    ...
    <div class="row">
        <div class="four col-md-2">
            <a href="#">
                <div class="counter-box colored "> <i class="fa-solid fa-list-check"></i> <span class="counter">{{\App\Models\Order::where('payment_method','cash')
                ->wherein('status',["pending","accept","done","shipping"])->count()}}</span>
        ...
        <div class="four col-md-2">
            <a href="#">
                <div class="counter-box colored "> <i class="fa-solid fa-list-check"></i>
                    <span class="counter">{{\App\Models\Order::where('payment_method','cash')
                    ->wherein('status',["pending","accept","done","shipping"])->sum('total_price')}}</span>
    ...
    <script>
        ...
        myDataTableColumns({
            name   :  ['id',  'shipping_address.name', 'shipping_address.phone','shipping_address.address', 'products_count',
                'total_price','discount','coupon_code','coupon_id','status'/*,'imaget','producttitles'*/],
