...
@if(!$product->has_paid_variant)
    <div class="form-group col-md-6 @if($product->is_order == 0 ) d-none @endif" id="day_order_dev">
        <label for="day_order"> @lang('form.label.day_order')</label>
        <input name="day_order" type="number" class="form-control @error('day_order') is-invalid @enderror" id="day_order"  value="{{old('day_order', $product->day_order)}}" step="any" >
    ...
    <div class="form-group col-md-6">
        <label for="sale_price"> @lang('form.label.price discount') @lang('form.label.optional')  --}}
{{--<span class=" p-1 cursor-pointer btn-outline-info"  data-toggle="modal" data-target="#fadeinModal">@lang('form.label.schedule sale')</span> --}}{{--
</label>
