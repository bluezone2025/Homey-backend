...
class OrderController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        if ($request->get('payment_method') == "cash" || $request->get('payment_method') == "wallet" ){
            if ($order->user){
                ...
                if($order->user_id != null){
                    ...
                    try{
                        ...
                        if (($FcmTokenModel && isset($FcmTokenModel->token )) || $request->get('fcm_token')){
                            self::save_notf($FcmTokenModel->token??$request->get('fcm_token'),false ,'Order',$order->id ,1,$order,$order->user_id);
                        }*/
                ...
                /*
                    ...
                    if($old_notf){
                        if(count($old_notf->fcm_token) != 0){
                            self::save_notf($old_notf->fcm_token[0],false ,'Order',$order->id ,1);
                        }
                    }
                }*/
...
//dd($product );
            ...
            if ($quantityProduct > $product->quantity) {  // assuming `available_quantity` is the actual quantity of the product
                //dd($product->name);
            ...
            if ($this->updateQuantityOfOptionValue($optionValueId , $quantityProduct,$product) === false) {
            ...
            // save product_order;
            $this->saveProductOrder($product , $quantityProduct,$colorId);
...
//dd($optionValue);
            ...
            //$optionValueId = substr($optionValueId,0,-1);
    ...
    private function updateQuantityOfProduct($quantityProduct, $product)
    {
        ...
        if ($quantityProduct && $product->quantity >= $quantityProduct) {
            ...
            if (\request()->get('payment_method') != "knet"){
                $product->update([
    ...
    */
    ...
    private function updateQuantityOfOptionValueOld($optionValueId, $quantityProduct,$product)
    {
        ...
        if ($optionValueId) {
          if(!$product->is_clothes){
            ...
            foreach (explode('-', $optionValueId) as $optionValue) {
                $data = explode('&',$optionValue);
                $optionValueRow = OptionValue::where('id', $data[0])->first();
                $quantityProduct = $data[1];
                //dd($optionValueRow);
            ...
            $attr=explode(',', $optionValueId);
            //dd($attr);
            $ProdColor=ProdColor::where('product_id',$product->id)->where('color_id',$attr[1])->where('size_id',$attr[0])->first();
            //dd($ProdColor);
            if ($ProdColor && $ProdColor->quantity >= $quantityProduct) {
                if (\request()->get('payment_method') != 'knet'){
                    $ProdColor->quantity = $ProdColor->quantity - $quantityProduct;
                    $ProdColor->save();
                }
                //$ProdColor->quantity = $ProdColor->quantity - $quantityProduct;
                //$ProdColor->save();
    ...
    private function updateQuantityOfOptionValue($optionValueId, $quantityProduct, $product)
    {
        if ($optionValueId) {
            ...
            if (!$product->is_clothes) {
                foreach (explode('-', $optionValueId) as $optionValue) {
                    ...
                    if ($optionValueRow && $optionValueRow->quantity >= $quantityProduct) {
                        if (request()->get('payment_method') != 'knet' || request()->get('payment_method') != 'tabby') {
                            ...
                            // Update quantity for related products' option values
                            foreach ($relatedProducts as $relatedProduct) {
                                $relatedOptionValue = OptionValue::where('product_id', $relatedProduct->id)
                                    ->where('id', $data[0])
                                    ->first();
                                if ($relatedOptionValue) {
                                    $relatedOptionValue->quantity -= $quantityProduct;
                                    $relatedOptionValue->save();
                                }
                            }
                        }
                        ...
                        // Handle points
                        $points_percentage = Cache::remember('points_percentage', 60, function () {
                            return Setting::where('name', 'points_percentage')->first()->value;
                        });
                ...
                $ProdColor = ProdColor::where('product_id', $product->id)
                    ->where('color_id', $attr[1])
                    ->where('size_id', $attr[0])
                    ->first();
                ...
                if ($ProdColor && $ProdColor->quantity >= $quantityProduct) {
                    if (request()->get('payment_method') != 'knet' || request()->get('payment_method') != 'tabby') {
                        ...
                        // Update quantity for related products' ProdColor
                        foreach ($relatedProducts as $relatedProduct) {
...
//dd(strpos($student_id, "=>$product->id"));
    ...
    private function saveProductOrder($product , $quantityProduct,$colorId =null){
    ...
    public function updateOrder(Request $request)
    {
      ...
      /*
        foreach ($items as $item){
          ...
          if (!$product->is_clothes){
              //$attribute_id = $attributes[0]['attribute']['id']?? null;
              $option_value_id = $attributes[0]['id']?? null;
              $optionValueRow = OptionValue::where('product_id', $product->id)->where('option_id',$option_value_id)->first();
              ...
              $ProdColor=ProdColor::where('product_id',$product->id)->where('color_id',$attributes[0]['id'])
                  ->where('size_id',$attributes[1]['id'])->first();
              $ProdColor->quantity = $ProdColor->quantity - $quantityProduct;
              $ProdColor->save();
          }
      ...
      */
        ...
        if ($order->user){
            try{
                ...
                if($order->user_id != null){
                    ...
                    //self::save_notf($FcmTokenModel->fcm_token,false ,'Order',$order->id ,2,$type_model=null,$order,$order->user_id);
                }
                */
            }catch (\Exception $e){
        ...
        /*if($request->fcm_token){
          self::save_notf($request->fcm_token,false ,'Order',$order->id ,2,$type_model=null,$order->user_id);
        }*/
